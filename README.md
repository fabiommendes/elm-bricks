...
elm-bricks is a library that decodes django-bricks Json into Elm Html elements. This makes it
possible to send bricks from the server to the client that are controlled by Elm's mainloop.
Elm-bricks also interacts via ports with the JS part of the bricks library.
